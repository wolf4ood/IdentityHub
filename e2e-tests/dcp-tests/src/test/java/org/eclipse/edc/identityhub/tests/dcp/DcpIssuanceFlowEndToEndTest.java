...
public class DcpIssuanceFlowEndToEndTest {
    ...
    abstract static class Tests {
        ...
        private static String PARTICIPANT_TOKEN = null;
        private static String ISSUER_DID = null;
        private static String PARTICIPANT_DID = null;
        ...
        static void beforeAll(IssuerServiceEndToEndTestContext issuer, IdentityHubEndToEndTestContext credentialService) {
            ...
            ISSUER_DID = issuer.didFor(ISSUER_ID);
            issuer.createParticipant(ISSUER_ID, ISSUER_DID);
            PARTICIPANT_DID = credentialService.didFor(PARTICIPANT_ID);
            PARTICIPANT_TOKEN = credentialService.createParticipant(PARTICIPANT_ID, PARTICIPANT_DID);
        }
        ...
        void issuanceFlow(IssuerServiceEndToEndTestContext issuer, IdentityHubEndToEndTestContext credentialService) {
            ...
            var credentialRequestStore = credentialService.getRuntime().getService(HolderCredentialRequestStore.class);
            var issuanceProcessStore = issuer.getRuntime().getService(IssuanceProcessStore.class);

            participantService.createParticipant(new Participant(PARTICIPANT_ID, PARTICIPANT_DID, "Participant"));
            ...
            credentialService.getIdentityApiEndpoint().baseRequest()
                    .contentType(JSON)
                    .header(new Header("x-api-key", PARTICIPANT_TOKEN))
                    .body(request)
                    .post("/v1alpha/participants/%s/credentials/request".formatted(toBase64(PARTICIPANT_ID)))
                    .then()
            ...
            await().pollInterval(Duration.ofSeconds(1))
                    ...
                    .untilAsserted(() -> {
                        var requests = credentialRequestStore.query(QuerySpec.max());
            ...
            await().pollInterval(Duration.ofSeconds(1))
                    .atMost(Duration.ofSeconds(60))
                    .untilAsserted(() -> {
                        var requests = issuanceProcessStore.query(QuerySpec.max());
                        assertThat(requests).hasSize(1)
                                .allSatisfy(t -> {
    ...
    class InMemory extends Tests {
        ...
        @RegisterExtension
        static IssuerServiceEndToEndExtension issuerService;

        @RegisterExtension
        static IdentityHubEndToEndExtension credentialService;

        static {
            issuerService = new IssuerServiceEndToEndExtension.InMemory();
            credentialService = new IdentityHubEndToEndExtension.InMemory();
        }
